...
CROP_RECOMMENDATION_CSV = os.path.join(
    BASE_DIR,
    "Crop_recommendation.csv"
)

CROP_YIELD_CSV = os.path.join(
    BASE_DIR,
    "crop_yield.csv"
)
...
X_train[cols_to_scale] = scaler_crop.fit_transform(
    X_train[cols_to_scale]
)

X_test[cols_to_scale] = scaler_crop.transform(
...
print(
    f"Crop Recommendation Model trained successfully "
...
for feature in categorical_features:
    ...
    X_train_y[feature] = label_encoders[feature].fit_transform(
        X_train_y[feature]
    )

    X_test_y[feature] = label_encoders[feature].transform(
        X_test_y[feature]
    )
...
X_train_y[numeric_features] = scaler_yield.fit_transform(
    X_train_y[numeric_features]
)

X_test_y[numeric_features] = scaler_yield.transform(
    X_test_y[numeric_features]
)
...
print(
    f"Crop Yield Model trained "
    f"(R²: {r2:.3f}, RMSE: {np.sqrt(mse):.3f})"
)
...
def predict_yield(input_data: dict) -> float:
    ...
    try:
        ...
        for feature in categorical_features:
            ...
            processed_data[feature] = label_encoders[
                feature
            ].transform([value])[0]
        ...
        df_input[numeric_features] = scaler_yield.transform(
            df_input[numeric_features]
        )
...
def home():
    ...
    return jsonify({
        "message": "KrishiMitra ML Backend is running successfully!..."
    })
...
def predict_yield_route():
    ...
    try:
        ...
        return jsonify({
            "status": "success",
            "predicted_yield":
                f"{prediction} metric tons per hectare"
        })
